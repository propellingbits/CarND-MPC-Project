 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
   ...
   for (int t = 0; t < N; t++) {
      ...
      fg[0] += 5000*CppAD::pow(vars[cte_start + t] - ref_cte, 2); //test out these 2k multiples
    ...
    for (int t = 0; t < N-1; t++) {
      ...
      // calculating cte and psi, and model constraints
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332*Lf; // 25 degrees
